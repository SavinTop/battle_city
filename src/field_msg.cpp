...
void FieldMessage::from_string(const std::string &s)
{
    if (s.size() < def_msg_size)
        _throw("the size is to small, no requared fields here");
    const int fld_cnt = cast(e_fields::field_cnt);
    std::stringstream ss;
    ss << s;

    FieldMessage new_msg; // if we have the exception, we dont lose this* if we use this var instead
    ...
    if (new_msg.msg_size != s.size())
        _throw("the size of the message is wrong");
    ...
    for (int i = 1; temp_bitmap; temp_bitmap >>= 1, i++)
    {
        ...
        if (static_cast<unsigned char>(exp_type) != type)
            _throw("the type of the fields do not match");

        if (exp_type == field_type::int32)
        {
            int av = ss.tellp();
            if (s.size() - av - 1 < sizeof(int32_t))
                _throw("there is not enough space in the buffer to read int value");
            int temp;
        ...
        else
        {
            if (s.size() - ss.tellp() - 1 < sizeof(uint16_t))
                _throw("there is not enough space in the buffer to get string size");
            uint16_t temp_size;
            ss.read(reinterpret_cast<char *>(&temp_size), sizeof(uint16_t));
            if (s.size() - ss.tellp() - 1 < temp_size)
                _throw("there is not enough space in the buffer to read string");
